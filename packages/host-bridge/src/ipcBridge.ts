...
export const dialog = {
  showOpen: bridge.buildProvider<
    IBridgeResponse<IOpenDialogResult>,
    | {
        defaultPath?: string;
...
export const mode = {
  fetchModelList: bridge.buildProvider<
    IBridgeResponse<{
      mode: Array<string | { id: string; name: string }>;
      fix_base_url?: string;
    }>,
    {
      base_url?: string;
...
/**
 * Ontology AI builder — session registry. The actual chat itself is a normal
 * Sudowork conversation created via `conversation.create`; this channel only
 * tracks which conversations were opened from the ontology workbench so the
 * AI 构建 tab can list them.
 */
export interface IOntologyAiBuilderSession {
  id: string;
  workspaceId: string;
  conversationId: string;
  title: string;
...
export interface IOntologyAiBuilderCreateSessionInput {
  workspaceId: string;
  conversationId: string;
  title?: string;
}
...
export const ontologyAiBuilder = {
   ...
   * `conversation.create`'s `extra.extraMcpConfigs`. Necessary because scode's
   * user-scope MCP is NOT auto-loaded into acp sessions — sessions only see
   * the MCPs passed to `session/new.mcpServers`.
   */
  ensureBuilderMcp: bridge.buildProvider<
    IBridgeResponse<{
      ready: boolean;
      mcpConfig: {
        name: string;
...
export const logs = {
  ...
  listLogFiles: bridge.buildProvider<
    IBridgeResponse<{
...
export const browserPanel = {
  ...
  evaluateScript: bridge.buildProvider<
    IBridgeResponse<{
      ok: boolean;
      value?: unknown;
      description?: string;
...
export const skillHub = {
  ...
  fetchSkills: bridge.buildProvider<
    IBridgeResponse<ISkillHubListResponse>,
    {
      cursor?: string;
      limit?: number;
      query?: string;
...
export const assistantHub = {
  ...
  fetchAssistants: bridge.buildProvider<
    IBridgeResponse<IAssistantHubListResponse>,
    {
      cursor?: string;
      limit?: number;
      query?: string;
...
export const dify = {
  ...
  uploadFile: bridge.buildProvider<
  ...
  textToAudio: bridge.buildProvider<
    IBridgeResponse<{ filePath: string; mimeType: string }>,
    IDifyAuthArgs & {
      messageId?: string;
      text?: string;
      voice?: string;
...
export const eeclaw = {
  ...
  uploadCustomSkill: bridge.buildProvider<
