# sudowork WebUI Mode - Startup Guide

sudowork supports WebUI mode, allowing you to access the application through a web browser. This guide covers how to start WebUI mode on all supported platforms.
...
WebUI mode starts sudowork with an embedded web server, allowing you to:

- Access the application through any modern web browser
- Use sudowork from remote devices on the same network (with `--remote` flag)
...
```cmd
# Using full path
"C:\Program Files\sudowork\sudowork.exe" --webui

# Or if sudowork is in your PATH
sudowork.exe --webui
```
...
open -a sudowork --args --webui
```

### Method 2: Create Shell Script

Create `start-sudowork-webui.sh`:
...
```bash
chmod +x start-sudowork-webui.sh
./start-sudowork-webui.sh
```
...
# Run with --webui flag
./sudowork-*.AppImage --webui
```

### Method 2: Create Desktop Entry

Create `~/.local/share/applications/sudowork-webui.desktop`:

```ini
[Desktop Entry]
Name=sudowork WebUI
Comment=Start sudowork in WebUI mode
Exec=/opt/sudowork/sudowork --webui
Icon=sudowork
Terminal=false
Type=Application
...
```bash
chmod +x ~/.local/share/applications/sudowork-webui.desktop
```
...
Create `~/bin/start-sudowork-webui.sh`:
...
```bash
chmod +x ~/bin/start-sudowork-webui.sh
```
...
Create `/etc/systemd/system/sudowork-webui.service`:
...
# Check status
sudo systemctl status sudowork-webui.service
```
...
**Important Note**: Electron desktop mode is **not supported** on Android. However, you can run sudowork in WebUI mode using Termux with a prooted Linux environment.

> **Community Contribution**: This guide is contributed by [@Manamama](https://github.com/Manamama). Special thanks for making sudowork accessible on Android devices! 🙏
>
> **Original Tutorial**: [Running sudowork WebUI on Android via Termux + Proot Ubuntu](https://gist.github.com/Manamama/b4f903c279b5e73bdad4c2c0a58d5ddd)
>
> **Related Issues**: [#217 - Android Support Discussion](https://github.com/sudoprivacy/sudowork/issues/217)
...
#### 4. Download and Install sudowork

```bash
# Download the ARM64 .deb package (replace VERSION with the actual version)
# Check latest version at: https://github.com/sudoprivacy/sudowork/releases
wget https://github.com/sudoprivacy/sudowork/releases/download/vVERSION/sudowork_VERSION_arm64.deb

# Example (replace VERSION with the release tag, e.g. v1.5.2):
wget https://github.com/sudoprivacy/sudowork/releases/download/vVERSION/sudowork_VERSION_arm64.deb

# Install the package
apt install -y ./sudowork_*.deb

# Verify installation
which sudowork
```

#### 5. Launch sudowork WebUI

```bash
# Start sudowork in WebUI mode with no-sandbox flag
sudowork --no-sandbox --webui
```
...
To access sudowork from other devices on your local network:

```bash
# Start with --remote flag
sudowork --no-sandbox --webui --remote
...
sudowork --no-sandbox --webui --port 8080
```
...
chmod +x /opt/sudowork/sudowork
```

#### Out of Memory

sudowork requires sufficient RAM. Close other apps if you encounter memory issues.

#### Cannot Access from Browser

1. Check if sudowork is running: look for "Server started" message
...
4. **Keep device charged** - Running sudowork consumes battery

### Tested Environment

- **Device**: Android 14
- **Termux Version**: 0.118.0
- **sudowork Version**: Latest release (e.g. 1.5.2)
- **Proot-distro**: Ubuntu (latest)

### Creating a Startup Script

For convenience, create a script to launch sudowork quickly:

```bash
# Create script in Ubuntu (proot)
cat > ~/start-sudowork.sh << 'EOF'
#!/bin/bash
echo "Starting sudowork WebUI..."
sudowork --no-sandbox --webui --remote
EOF

# Make executable
chmod +x ~/start-sudowork.sh
...
```bash
proot-distro login ubuntu -- bash -c "sudowork --no-sandbox --webui --remote"
```
...
2. Report issues at [GitHub Issues #217](https://github.com/sudoprivacy/sudowork/issues/217)
...
netsh advfirewall firewall add rule name="sudowork WebUI" dir=in action=allow protocol=TCP localport=3000
```
...
**macOS:**
Go to **System Preferences** → **Security & Privacy** → **Firewall** → **Firewall Options** → Add sudowork
...
which sudowork
# or
find /opt -name "sudowork" 2>/dev/null
```
...
```powershell
& "C:\Program Files\sudowork\sudowork.exe" --webui 2>&1 | Tee-Object -FilePath sudowork.log
```

**macOS/Linux:**

```bash
/path/to/sudowork --webui 2>&1 | tee sudowork.log
```
...
export sudowork_PORT=8080

# Allow remote access without passing --remote
export sudowork_ALLOW_REMOTE=true

# Optional host hint (0.0.0.0 behaves the same as sudowork_ALLOW_REMOTE=true)
export sudowork_HOST=0.0.0.0
...
```cmd
# Using full path
"C:\Program Files\sudowork\sudowork.exe" --resetpass

# Or for a specific user
"C:\Program Files\sudowork\sudowork.exe" --resetpass username
```
...
```bash
# Using full path
/Applications/sudowork.app/Contents/MacOS/sudowork --resetpass

# Or for a specific user
/Applications/sudowork.app/Contents/MacOS/sudowork --resetpass username
```
...
# Or for a specific user
sudowork --resetpass username

# Or using full path
/opt/sudowork/sudowork --resetpass
```
...
- [GitHub Issues](https://github.com/sudoprivacy/sudowork/issues)
...
2. Search [existing issues](https://github.com/sudoprivacy/sudowork/issues)
3. Create a [new issue](https://github.com/sudoprivacy/sudowork/issues/new) with:
   - Your OS and version
   - sudowork version
   - Steps to reproduce
...
**Happy using sudowork in WebUI mode!** 🚀
