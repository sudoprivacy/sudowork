...
if (brand.BUILD_OFFLINE !== true) {
  module.exports = { extends: './electron-builder.yml', ...branding, win: { legalTrademarks: copyright } };
} else {
  const config = parse(fs.readFileSync('./electron-builder.yml', 'utf8'));
  const withoutRuntime = (entries = []) => entries.filter((entry) => {
  ...
  config.linux = { ...config.linux, extraResources: [...withoutRuntime(config.linux?.extraResources), ...runtimeResources('linux')] };
