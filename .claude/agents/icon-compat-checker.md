...
### 1. Arco Button 的 icon prop 优先用 Arco 官方 icon

当代码是 Arco Button，并且图标有 Arco 官方等价项时：
...
### 4. IconPark 不用于 Arco Button icon

发现 Arco Button 里使用 IconPark：
...
3. 不通过补 `.i-icon + span` 间距来继续保留 IconPark，除非用户明确要求保留 IconPark。

### 5. lucide-react 放入 Arco Button 通常可行，但要检查风格一致性
...
但如果同一页面大部分按钮图标来自 Arco 官方 icon，只在一个按钮使用 lucide，需检查视觉风格是否突兀。能用 Arco 官方 icon 时，优先换成 Arco 官方 icon。
...
3. 逐个检查 Arco `Button icon={...}`：
   ...
   - 如果使用 lucide，确认是否有风格统一要求；无明显问题不要为了换而换。
4. 检查普通图标 + 文本组合：
