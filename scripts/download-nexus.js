...
const PLATFORMS = {
  'darwin-arm64': { name: 'nexus-cluster-macos-arm64', outputFile: 'nexusd' },
  'darwin-x64': { name: 'nexus-cluster-macos-x86_64', outputFile: 'nexusd' },
  'win32-x64': { name: 'nexus-cluster-windows-x86_64.exe', outputFile: 'nexusd.exe' },
  'win32-arm64': { name: 'nexus-cluster-windows-x86_64.exe', outputFile: 'nexusd.exe' },
};
...
function getOutputFile(platform) {
  const config = PLATFORMS[platform];
  if (!config) throw new Error(`Unknown platform: ${platform}`);
  return path.join(RESOURCES_DIR, config.outputFile);
}
...
async function main() {
  ...
  if (!platform) {
    ...
    if (PLATFORMS[currentPlatform]) {
      ...
      // Create empty placeholder file so electron-builder doesn't fail
      fs.mkdirSync(RESOURCES_DIR, { recursive: true });
      const fallbackOutput = path.join(RESOURCES_DIR, process.platform === 'win32' ? 'nexusd.exe' : 'nexusd');
  ...
  try {
    ...
    // Create empty placeholder file so electron-builder doesn't fail
    fs.mkdirSync(RESOURCES_DIR, { recursive: true });
    const fallbackOutput = getOutputFile(platform);
    fs.writeFileSync(fallbackOutput, Buffer.alloc(0));
    clearLocalDevRuntimeVersion('nexus');
