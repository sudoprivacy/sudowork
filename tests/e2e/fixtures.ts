 ...
 *      (e.g. out/linux-unpacked/aionui, out/mac-arm64/Sudowork.app, out/win-unpacked/Sudowork.exe).
...
function resolvePackagedApp(): { executablePath: string; cwd: string } | null {
  ...
  if (platform === 'win32') {
    // out/win-unpacked/Sudowork.exe  or  out/win-x64-unpacked/Sudowork.exe
    ...
    // out/mac-arm64/Sudowork.app/Contents/MacOS/Sudowork  or  out/mac/Sudowork.app/...
    ...
    for (const dir of ['linux-unpacked', 'linux-x64-unpacked', 'linux-arm64-unpacked']) {
      ...
      for (const name of ['aionui', 'Sudowork']) {
