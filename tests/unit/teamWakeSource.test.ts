...
const ALL_SOURCES: WakeSource[] = ['user_message', 'user_intervention', 'mcp_send_message', 'idle_notification', 'interrupted_notification', 'spawn_welcome', 'mcp_shutdown_request', 'crash_notification', 'inactivity_timeout', 'spawn_attach_failure', 'shutdown_rejected', 'recovery_drain'];
...
describe('WakeSource taxonomy (附录 I.2)', () => {
  ...
  it('Background neither resumes nor bypasses pause', () => {
    for (const s of ['mcp_send_message', 'idle_notification', 'interrupted_notification'] as WakeSource[]) {
