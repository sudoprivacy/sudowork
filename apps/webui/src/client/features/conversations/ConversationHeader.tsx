...
export function ConversationHeader({
  ...
  const currentName =
    models.find((m) => m.id === currentModel || m.id === currentModel?.replace(/^proxy\//, ''))
      ?.name ?? (currentModel ? currentModel.replace(/^proxy\//, '') : (models[0]?.name ?? '模型'))
  return (
                ...
                <Menu.Item key={m.id} onClick={() => onSetModel(m.id)}>
                  <span className="flex items-center gap-2">
                    <span
                      ...
                      style={{
                        background:
                          m.name === currentName ? 'var(--primary)' : 'var(--color-text-4)',
                      }}
                    />
