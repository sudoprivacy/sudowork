...
export function CronJobDetailPage(): React.ReactElement {
  ...
  return (
    <div
      ...
      data-testid="cron-detail-page"
    >
      <div className="page-content mx-auto w-full max-w-240">
        {/* 返回 */}
        <div
          className="inline-flex items-center gap-1 text-13px text-secondary hover:text-foreground hover:bg-base rd-2 px-2 py-1 mb-4 -ml-2 cursor-pointer"
          onClick={() => void navigate('/cron')}
        >
          ← 全部定时任务
        </div>
        ...
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="flex flex-col gap-0.5 min-w-0">
            <h2 className="text-24px font-600 text-foreground my-0 truncate">
              {job?.name ?? '任务详情'}
            </h2>
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-13px text-secondary">
                下次运行 {job?.nextRunAt ? new Date(job.nextRunAt).toLocaleString() : '—'}
              </span>
            </div>
          </div>
        ...
        <div className="space-y-6 mt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <InfoField
              label="描述"
              value={String((job as { description?: string } | undefined)?.description ?? '—')}
            />
            <InfoField
              label="执行模式"
              value={job?.conversationMode === 'reuse' ? '复用已有会话' : '每次新建会话'}
            />
            <div>
