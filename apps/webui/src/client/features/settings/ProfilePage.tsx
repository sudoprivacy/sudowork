      ...
      <div className="max-w-2xl mx-auto flex flex-col gap-4">
        ...
        <Descriptions
          ...
          data={[
            {
              label: '用户名',
              value: String(user.name ?? user.username ?? user.displayName ?? '—'),
            },
            { label: '用户 ID', value: String(user.id ?? user.userId ?? '—') },
            {
              label: '部门',
              value: String(user.departmentName ?? user.department ?? user.departmentId ?? '—'),
            },
            { label: '角色', value: String(user.role ?? '—') },
            {
              label: '累计 Token',
              value: formatNum(profile.totalTokens ?? profile.total_tokens ?? user.totalTokens),
            },
            {
              label: '会话数',
              value: formatNum(profile.sessionCount ?? profile.session_count ?? user.sessionCount),
            },
          ]}
        />
        <div className="text-12px text-tertiary">
          密码修改由管理员在 Moss 侧管理（当前版本无自助改密）。
        </div>
      </div>
    </div>
  )
