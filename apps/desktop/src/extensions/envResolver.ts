...
import { NEXUS_STRICT_ENV_ENV } from './constants';
...
export function isGlobalStrictMode(): boolean {
  if (_globalStrictMode === undefined) {
    _globalStrictMode = process.env[NEXUS_STRICT_ENV_ENV] === '1' || process.env[NEXUS_STRICT_ENV_ENV] === 'true';
  }
...
export function resolveEnvTemplates(value: string, options?: EnvResolverOptions): string {
  ...
  const result = value.replace(ENV_TEMPLATE_REGEX, (_match, varName: string) => {
    ...
    if (envValue === undefined) {
      ...
      if (strictMode) {
        throw new UndefinedEnvVariableError(varName, `[Extensions] Strict mode: Required environment variable "${varName}" is not defined. Set the variable or disable strict mode (NEXUS_STRICT_ENV=0).`);
      }
  ...
  if (!strictMode && undefinedVars.length > 0) {
    console.warn(`[Extensions] ${undefinedVars.length} undefined environment variable(s): ${undefinedVars.join(', ')}. Enable strict mode (NEXUS_STRICT_ENV=1) to catch these errors early.`);
  }
