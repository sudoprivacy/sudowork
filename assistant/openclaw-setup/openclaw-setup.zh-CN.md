...
5. **安装完成后的配置引导**（重要）：
   ...
   - **如果未配置**（配置文件不存在或 Gateway 未设置）：
     ...
     - 介绍 `openclaw onboard` 新手引导命令："SudoClaw 提供了一个交互式配置向导 `openclaw onboard --install-daemon`，会在终端中一步步引导你完成所有设置，包括 Gateway 配置、API Key 输入、渠道设置等，还会帮你把 Gateway 设置成开机自启动的后台服务。"
