...
import { isSafetyHookEnabled, onEnabledChange } from '@process/services/safety/SafetyPollingService';
...
/**
 * 获取 hook.js 的路径
 * Get path to hook.js for worker preloading
 */
function getHookJsPath(): string {
  if (app.isPackaged) {
    // 打包环境: hook.js 在 resources 目录
...
/**
 * Active worker processes for broadcasting safety hook toggle
 */
const activeWorkers: Set<UtilityProcess> = new Set();

/**
 * Broadcast safety hook toggle to all active workers
 */
function broadcastSafetyHookToggle(enabled: boolean): void {
  const message = { type: 'safety.hook.toggle', data: { enabled } };
  activeWorkers.forEach((worker) => {
    try {
      worker.postMessage(message);
    } catch (error) {
      console.error('[ForkTask] Failed to broadcast to worker:', error);
    }
  });
  console.log(`[ForkTask] Broadcasted safety.hook.toggle to ${activeWorkers.size} workers: enabled=${enabled}`);
}

/**
 * Broadcast blacklist config update to all active workers
 */
export function broadcastBlacklistUpdate(config: any): void {
  const message = { type: 'safety.blacklist.update', data: { config } };
  activeWorkers.forEach((worker) => {
    try {
      worker.postMessage(message);
    } catch (error) {
      console.error('[ForkTask] Failed to broadcast blacklist to worker:', error);
    }
  });
  console.log(`[ForkTask] Broadcasted blacklist update to ${activeWorkers.size} workers: rules=${config?.rules?.length || 0}`);
}

// Subscribe to safety hook enabled state changes (once, at module load)
let subscribedToEnabledChange = false;
if (!subscribedToEnabledChange) {
  onEnabledChange((enabled) => {
    broadcastSafetyHookToggle(enabled);
  });
  subscribedToEnabledChange = true;
}
...
export class ForkTask<Data> extends Pipe {
  ...
  protected init() {
    ...
    const workerEnv = getEnhancedEnv({
      // Pass safety hook enabled state to worker
      SUDOWORK_SAFETY_HOOK: isSafetyHookEnabled() ? 'true' : 'false',
    });

    // Build execArgv based on safety hook enabled state
    const execArgv: string[] = [];
    if (isSafetyHookEnabled()) {
      const hookJsPath = getHookJsPath();
      execArgv.push('-r', hookJsPath);
      console.log(`[ForkTask] Injecting safety hook: -r ${hookJsPath}`);
    }
    ...
    // Track active worker for broadcasting
    activeWorkers.add(fcp);
