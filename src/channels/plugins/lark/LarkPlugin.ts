...
export class LarkPlugin extends BasePlugin {
  ...
  private setupEventHandlers(): void {
    ...
    this.eventDispatcher.register({
      ...
      // Fire-and-forget: do NOT await — avoids blocking the Lark WebSocket event loop
      // during AI generation (10-30s), which would trigger Feishu's 3s timeout and retries.
      ...
      // Fire-and-forget: same reason as im.message.receive_v1
  ...
  static async testConnection(appId: string, appSecret?: string): Promise<{ success: boolean; botInfo?: { name?: string }; error?: string }> {
    ...
    try {
      ...
      // Note: Feishu API returns HTTP 200 even on auth failure,
      // with the error indicated in the response body's `code` field
