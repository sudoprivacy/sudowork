...
const tenantDataSchema = z.object({
  ...
  privacyPolicyUrl: z.union([z.literal(''), z.string().url()]),
...
export function resolveTenantDefaults() {
  ...
  return {
    ...
    loginDescription: brandConfig.tagline?.trim() ?? '',
    aboutName: displayName ?? BUILTIN_TENANT.aboutName,
    companyName: brandConfig.companyName?.trim() ?? '',
    websiteUrl: brandConfig.websiteUrl?.trim() ?? '',
    privacyPolicyUrl: brandConfig.privacyPolicyUrl?.trim() ?? '',
