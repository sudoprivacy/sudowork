...
export const SettingsList: React.FC<SettingsListProps> = ({ children, className }) => {
  return <div className={classNames('overflow-hidden rd-12px border border-solid border-[var(--border-default)] bg-bg-1', className)}>{children}</div>;
};
...
export const SettingsListItem: React.FC<SettingsListItemProps> = ({ icon, title, description, tag, status, action, className, children }) => {
  return (
    <div className={classNames('border-0 border-b border-solid border-[var(--border-default)] px-18px py-16px last:border-b-0', className)}>
      <div className='flex flex-wrap items-center gap-14px'>
        {icon && <div className='flex h-40px w-40px shrink-0 items-center justify-center rd-8px border border-solid border-[var(--border-default)] bg-fill-1 text-t-secondary'>{icon}</div>}
