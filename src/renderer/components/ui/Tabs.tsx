...
export default function Tabs({ items, value, onChange, onMouseDown, className, itemClassName, ariaLabel, variant = 'pill' }: ITabsProps) {
  return (
    <div role='tablist' aria-label={ariaLabel} className={classNames('flex flex-wrap', variant === 'pill' && 'gap-2', variant === 'line' && 'items-end gap-5 border-b border-border', className)}>
      {items.map((item) => {
        ...
        return (
          <button
            ...
            className={classNames(
              ...
              variant === 'pill' && ['h-7 border rd-full px-3 text-xs active:scale-96', isActive ? 'border-primary bg-primary font-semibold text-primary-foreground hover:bg-primary hover:text-primary-foreground hover:brightness-112' : 'bg-secondary text-foreground hover:bg-accent'],
              variant === 'line' && ['relative h-9 px-0 text-sm bg-transparent border-none', isActive ? 'text-primary font-medium' : 'text-foreground-secondary hover:text-foreground'],
