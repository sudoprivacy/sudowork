...
const ActionChip = forwardRef<HTMLButtonElement, ActionChipProps>(({ icon, label, active = false, disabled = false, onClick, className, title, ...rest }, ref) => {
  return (
    <button ref={ref} type='button' title={title} disabled={disabled} className={classNames('inline-flex h-8 min-w-0 items-center gap-2 rd-999px border border-solid px-12px text-13px font-500 transition-colors', 'bg-fill-2 text-secondary hover:bg-fill-3 hover:text-foreground', 'disabled:cursor-not-allowed disabled:opacity-55', active && 'border-[rgba(var(--ui-accent-orange-rgb),0.44)] bg-[rgba(var(--ui-accent-orange-rgb),0.12)] text-[var(--ui-accent-orange)] hover:bg-[rgba(var(--ui-accent-orange-rgb),0.16)]', onClick ? 'cursor-pointer' : 'cursor-default', className)} onClick={onClick} {...rest}>
