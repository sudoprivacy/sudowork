...
const PolicyTab: React.FC<PolicyTabProps> = ({ policy, loading = false }) => {
  ...
  return (
    <div className='flex flex-col gap-0.5 rd-12px bg-1 overflow-hidden'>
      {KNOWN_ROWS.map((row) => (
        <div key={row.key} className='flex items-center justify-between gap-4 px-4 py-3.5 border-b border-solid border-[var(--border-default)] last:border-b-0'>
      ...
      {extraRows.map(([key, value]) => (
        <div key={key} className='flex items-center justify-between gap-4 px-4 py-3.5 border-b border-solid border-[var(--border-default)] last:border-b-0'>
          <div className='flex-1 min-w-0'>
