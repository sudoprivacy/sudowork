    ...
    <Modal title={t('settings.changePassword.title', '修改密码')} visible={visible} onOk={handleSubmit} onCancel={onClose} okText={t('settings.changePassword.confirm', '确认修改')} cancelText={t('settings.changePassword.cancel', '取消')} confirmLoading={loading}>
      <div className='flex flex-col gap-4'>
        <div className='flex flex-col gap-2'>
          <div className='ml-1 text-12px font-600 text-foreground-secondary'>{t('settings.changePassword.oldPasswordLabel', '原始密码')}</div>
        ...
        <div className='flex flex-col gap-2'>
          <div className='ml-1 text-12px font-600 text-foreground-secondary'>{t('settings.changePassword.newPasswordLabel', '新密码')}</div>
          ...
          <div className='ml-1 text-12px text-foreground-secondary'>{t('settings.changePassword.ruleHint', '8-20 位，需同时包含大写字母、小写字母和数字。')}</div>
        </div>
      </div>
    </Modal>
  );
