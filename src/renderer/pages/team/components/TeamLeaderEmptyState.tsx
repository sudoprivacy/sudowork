...
export default function TeamLeaderEmptyState({ assistantName, assistantAvatar, assistantBackend, assistantId, source, onPromptClick }: ITeamLeaderEmptyStateProps) {
  ...
  return (
    <div className='w-full flex justify-center'>
      <div className='w-full max-w-560px rd-24px border border-light bg-(--color-bg-2) px-28px py-30px text-center shadow-[0_12px_32px_rgba(15,23,42,0.05)] animate-fade-in animate-duration-300 animate-ease-out'>
        ...
        <div className='flex flex-col gap-12px text-left'>
          {PROMPTS.map((prompt) => {
            ...
            return (
              ...
              >
                <span className='h-32px w-32px shrink-0 rd-10px bg-fill-2 f-center text-17px leading-none transition-colors group-hover:bg-fill-3'>{prompt.icon}</span>
