...
export default function TeamWarmupOverlay({ phase, members, runtimeStatus, error, onRetry }: ITeamWarmupOverlayProps) {
  ...
  return (
    <div className='absolute inset-0 z-20 flex items-center justify-center bg-[rgba(var(--gray-1),0.78)] backdrop-blur-[3px]'>
      <div className='flex max-w-420px flex-col items-center gap-14px rounded-18px border border-[var(--color-border-2)] bg-1 px-36px py-28px shadow-[0_8px_30px_rgba(0,0,0,0.08)]'>
        <div className='flex max-w-320px flex-wrap items-center justify-center gap-10px'>
          {members.slice(0, 8).map((member) => {
            ...
            return (
              <span
                key={member.slot_id}
                className={`inline-flex size-36px items-center justify-center rounded-full border bg-fill-1 transition-all ${isFailed ? 'border-red-500 grayscale' : isReady ? 'border-green-500 shadow-[0_0_0_3px_rgba(34,197,94,0.12)]' : 'border-[var(--color-border-2)] opacity-50'}`}
              >
