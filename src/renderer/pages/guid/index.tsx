...
const GuidPage: React.FC = () => {
  ...
  const skillTriggerNode = (
    ...
    >
      <ActionChip icon={<AtSign size={14} />} label={t('conversation.welcome.skill', '技能')} onClick={() => setIsSkillPopoverOpen(true)} />
    </SkillSelectorPopover>
  );
        ...
        {isAssistantMode ? (
          ...
          <>
            ...
            <div className='w-full px-4 py-3 mb-4 rd-2xl text-sm border box-border animate-fade-in animate-duration-400 animate-ease-out'>
              {selectedAssistantConfig.descriptionI18n?.[localeKey] || selectedAssistantConfig.description || t('settings.assistantDescriptionPlaceholder', '帮你解决什么问题')}
            </div>
          </>
        ) : (
          /* ========== Normal Mode ========== */
        ...
        {isAssistantMode ? (
          ...
          assistantPrompts.length > 0 && (
            <div className='mt-4 w-full animate-fade-in animate-duration-400 animate-ease-out'>
              <div className='text-13px mb-2 text-tertiary'>{t('guid.trySuggestionPrompts', '试试这些可点击的案例提示词：')}</div>
