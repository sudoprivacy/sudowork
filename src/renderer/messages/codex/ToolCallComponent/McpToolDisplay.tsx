    ...
    <BaseToolCallDisplay toolCallId={toolCallId} title={getDisplayTitle()} status={status} description={description} icon='🔌'>
      ...
      {toolDetails && (
        <div className='text-sm mb-2'>
          <div className='mb-1 text-xs text-foreground-secondary'>{t('tools.labels.tool_details')}</div>
          <div className='rounded-md border border-border bg-muted p-2 text-sm'>
            ...
            {toolDetails.arguments && (
              <div className='mt-2'>
                <div className='mb-1 text-xs text-foreground-secondary'>{t('tools.labels.arguments')}</div>
                <pre className='overflow-x-auto rounded-md border border-border bg-secondary p-2 text-xs text-foreground'>{JSON.stringify(toolDetails.arguments, null, 2)}</pre>
              </div>
            )}
          </div>
        </div>
      ...
      {subtype === 'mcp_tool_call_end' && data?.result && (
        <div className='text-sm mb-2'>
          <div className='mb-1 text-xs text-foreground-secondary'>{t('tools.labels.result')}</div>
          <div className='max-h-40 overflow-y-auto rounded-md border border-border bg-muted p-2 text-sm'>
