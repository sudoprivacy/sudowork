    ...
    <BaseToolCallDisplay toolCallId={toolCallId} title={getDisplayTitle()} status={status} description={description} icon='🔌'>
          ...
          <div className='bg-1 p-2 rounded text-sm border border-solid border-[var(--border-default)]'>
            ...
            {toolDetails.arguments && (
              <div className='mt-2'>
                ...
                <pre className='text-xs bg-2 p-2 rounded border border-solid border-[var(--border-default)] overflow-x-auto text-t-primary'>{JSON.stringify(toolDetails.arguments, null, 2)}</pre>
              </div>
            )}
          </div>
        </div>
      ...
      {subtype === 'mcp_tool_call_end' && data?.result && (
        <div className='text-sm mb-2'>
          ...
          <div className='bg-1 p-2 rounded text-sm max-h-40 overflow-y-auto border border-solid border-[var(--border-default)]'>
