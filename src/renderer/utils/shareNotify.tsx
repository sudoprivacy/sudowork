...
export function showShareLoading(options?: ShareNotifyOptions): string {
  Notification.info({
    ...
    content: React.createElement('div', { className: 'flex items-center gap-8px' },
      React.createElement('span', { className: 'arco-notification-loading-icon' }),
      React.createElement('span', null, '分享中...')
    ),
...
export function updateShareError(id: string, errorMsg: string): void {
  Notification.error({
    id: id || SHARE_NOTIFY_ID,
    title: 'ShareOne',
    content: React.createElement('div', { className: 'flex flex-col gap-4px' },
      React.createElement('span', { className: 'text-13px color-error font-500' }, '分享失败'),
      React.createElement('span', { className: 'text-12px color-text-3 break-all max-w-200px' }, errorMsg)
    ),
    duration: 8000,
    closable: true,
    position: 'topRight',
...
/**
 * Close share notification
 */
export function closeShareNotify(id?: string): void {
  Notification.remove(id || SHARE_NOTIFY_ID);
}