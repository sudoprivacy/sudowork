  ...
  initialModePromise = getAppMode()
    .then((mode) => {
      ...
      if (mode === null && localStorage.getItem('sudowork_auth_v2')) {
        setAppMode('c', { orchestrate: false }).catch((e) => {
...
export function resetAppModeCache(mode?: 'c' | 'e'): void {
  initialModeResolved = mode !== undefined;
  appModeWasNull = false;
  cachedMode = mode ?? null;
  initialModePromise = mode ? Promise.resolve(mode) : null;
}
