...
export async function loadCliConfig({ workspace, settings, extensions, sessionId, proxy, model, conversationToolConfig, yoloMode, mcpServers, skillsDir, enabledSkills }: LoadCliConfigOptions): Promise<Config> {
  ...
  const config = new Config({
    ...
    // Sudowork 是桌面应用，支持用户交互确认，需要设置 interactive: true
    // Sudowork is a desktop app with user interaction support, needs interactive: true
    interactive: true,
    ...
    // Sudowork uses its own skill mechanism (AcpSkillManager) with plain-text index injection
    skillsSupport: false,
