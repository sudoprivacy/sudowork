...
export class ServiceManager {
  ...
  async startup(): Promise<void> {
    ...
    try {
      ...
      // Provision the sudowork-owned aidb browser dispatcher into
      // ~/.nexus/sudoclaw/bin. The scode runtime surfaces this dir on PATH
      // (acpConnectors), so the browser tool depends on it even though the
      // legacy gateway daemon is gone. Best-effort: never block startup.
      try {
