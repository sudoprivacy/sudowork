...
async function reloadSkillRuntime(): Promise<void> {
  // Clear cached skill metadata and reset the ACP skill manager so the next
  // scode session picks up the updated skills from disk.
  clearSkillsCache();
  AcpSkillManager.resetInstance();
  mainLog('SkillHub', 'Cleared skill cache; updated skills will load on next session');
}
...
export function initSkillHubBridge(): void {
  ...
  ipcBridge.skillHub.downloadAndInstallSkill.provider(async ({ skillName, displayName, sourceUrl, version, checksum, skillMeta }) => {
    try {
      ...
      // Clear the skill cache so the next scode session picks up new skills.
      void (async () => {
        try {
          await reloadSkillRuntime();
        } catch (err) {
