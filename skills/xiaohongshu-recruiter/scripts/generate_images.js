...
// Priority: 1) NEXUS_FONTS_DIR env var, 2) skills/canvas-design relative path, 3) system fonts
function getFontDir() {
  const candidates = [process.env.NEXUS_FONTS_DIR, path.join(__dirname, '../../canvas-design/canvas-fonts'), path.join(process.env.HOME || '', 'Library/Application Support/Sudowork/config/skills/canvas-design/canvas-fonts'), path.join(process.env.APPDATA || '', 'Sudowork/config/skills/canvas-design/canvas-fonts')].filter(Boolean);
